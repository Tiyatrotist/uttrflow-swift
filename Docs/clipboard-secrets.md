...
|---|---|---|
| `org.nspasteboard.ConcealedType` | a password or similar | keeps the clip for the running app as `.secret`, whatever its shape, and never writes it to the history file |
| `org.nspasteboard.TransientType` | a copy made to move data, not for history | does not record it, so a password manager that sets only this marker leaves no panel row and no history |
